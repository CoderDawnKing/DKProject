...
- (void)handleShowContentView {
    ...
    contentView.backgroundColor = UIColor.dk_backgroundColorLighten;
...
- (void)handleCustomDimmingView {
    ...
    contentView.layer.shadowColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, __kindof NSObject * _Nullable theme) {
        return [identifier isEqualToString:DKThemeIdentifierDark] ? UIColorWhite : UIColorBlack;
    }].CGColor;
    ...
    if ([QMUIThemeManagerCenter.defaultThemeManager.currentThemeIdentifier isEqual:DKThemeIdentifierDark]) {
        blurredBackgroundImage = [DKUIImageEffects imageByApplyingDarkEffectToImage:blurredBackgroundImage];
    } else {
        blurredBackgroundImage = [DKUIImageEffects imageByApplyingExtraLightEffectToImage:blurredBackgroundImage];
    }
...
- (void)handleLayoutBlockAndAnimation {
    ...
    NSMutableParagraphStyle *paragraphStyle = [NSMutableParagraphStyle qmui_paragraphStyleWithLineHeight:24];
    paragraphStyle.paragraphSpacing = 16;
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:@"利用layoutBlock可以自定义浮层的布局，注意此时contentViewMargins、maximumContentViewWidth属性均无效，如果需要实现外间距、最大宽高的保护，请自行计算。\n另外搭配showingAnimation、hidingAnimation也可制作自己的显示/隐藏动画，例如这个例子里实现了一个从底部升起的面板，升起后停靠在容器底端，你可以试着旋转设备，会发现依然能正确布局。" attributes:@{NSFontAttributeName: UIFontMake(16), NSForegroundColorAttributeName: UIColor.dk_mainTextColor, NSParagraphStyleAttributeName: paragraphStyle}];
...
- (void)handleKeyboard {
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:@"如果你的浮层里有输入框，建议在把输入框添加到界面上后立即调用becomeFirstResponder（如果你用contentViewController，则在viewWillAppear:时调用becomeFirstResponder），以保证键盘跟随浮层一起显示。\n而在浮层消失时，modalViewController会自动降下键盘，所以你的浮层里并不需要处理。" attributes:@{NSFontAttributeName: UIFontMake(12), NSForegroundColorAttributeName: UIColor.dk_mainTextColor, NSParagraphStyleAttributeName: paragraphStyle}];
    
    NSDictionary *codeAttributes = @{NSFontAttributeName: CodeFontMake(12), NSForegroundColorAttributeName: UIColor.dk_codeColor};
    [attributedString.string enumerateCodeStringUsingBlock:^(NSString *codeString, NSRange codeRange) {
        [attributedString addAttributes:codeAttributes range:codeRange];
    }];
...
- (void)handleShowInView {
    if (!self.modalViewControllerForAddSubview) {
        ...
        NSMutableParagraphStyle *paragraphStyle = [NSMutableParagraphStyle qmui_paragraphStyleWithLineHeight:24];
        paragraphStyle.paragraphSpacing = 16;
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:@"QMUIModalPresentationViewController支持 3 种使用方式，当前使用第 3 种，注意可以透过遮罩外的空白地方点击到背后的 cell。\n这种方式适用于需要保持浮层显示的情况下切换界面，你可以点击按钮看效果。" attributes:@{NSFontAttributeName: UIFontMake(16), NSForegroundColorAttributeName: UIColor.dk_mainTextColor, NSParagraphStyleAttributeName: paragraphStyle}];
        NSDictionary *codeAttributes = CodeAttributes(16);
        [attributedString.string enumerateCodeStringUsingBlock:^(NSString *codeString, NSRange codeRange) {
            [attributedString addAttributes:codeAttributes range:codeRange];
        }];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = UIColor.dk_backgroundColorLighten;
    ...
    self.imageView.layer.borderColor = UIColor.dk_separatorColor.CGColor;
