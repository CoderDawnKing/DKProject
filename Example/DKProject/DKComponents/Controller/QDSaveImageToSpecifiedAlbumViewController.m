...
- (void)initSubviews {
    ...
    // 普通按钮
    self.changeImageButton = [DKUIHelper generateLightBorderedButton];
    ...
    // 边框按钮
    self.saveButton = [DKUIHelper generateDarkFilledButton];
...
- (UIImage *)randomImage {
    ...
    UIImage *resultBackgroundImage = [UIImage qmui_imageWithShape:QMUIImageShapeOval size:CGSizeMake(TestImageSize.width, TestImageSize.height) tintColor:[DKCommonUI randomThemeColor]];
        ...
        [QMUIAssetsManager requestAuthorization:^(QMUIAssetAuthorizationStatus status) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (status == QMUIAssetAuthorizationStatusAuthorized) {
                    ...
                    [DKUIHelper showAlertWhenSavedPhotoFailureByPermissionDenied];
                }
            });
        }];
