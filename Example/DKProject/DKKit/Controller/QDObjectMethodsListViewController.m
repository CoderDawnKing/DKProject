    ...
    void (^searchBlock)(NSString *obj, BOOL *stop) = ^(NSString *obj, BOOL *stop) {
        ...
        if (lastLocation != NSNotFound) {
            ...
            for (NSInteger i = 0; i < highlightedLocation.count; i++) {
                [result addAttribute:NSForegroundColorAttributeName value:DKThemeManager.currentTheme.themeCodeColor range:NSMakeRange(highlightedLocation[i].unsignedIntegerValue, 1)];
            }
