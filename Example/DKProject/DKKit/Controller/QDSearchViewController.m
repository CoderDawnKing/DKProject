...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.backgroundColor = UIColor.dk_backgroundColor;
        
        self.titleLabel = [[QMUILabel alloc] qmui_initWithFont:UIFontMake(14) textColor:UIColor.dk_mainTextColor];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.tableView) {
        ...
        if (range.location != NSNotFound) {
            [attributedString addAttributes:@{NSForegroundColorAttributeName: UIColor.dk_tintColor} range:range];
        }
