...
@interface DKBaseGridButton : QMUIButton

@end

@interface DKBaseGridViewController ()

@end

@implementation DKBaseGridViewController
...
- (DKBaseGridButton *)generateButtonAtIndex:(NSInteger)index {
    NSString *keyName = self.dataSource.allKeys[index];
    NSAttributedString *attributedString = [[NSAttributedString alloc] initWithString:keyName attributes:@{NSForegroundColorAttributeName: UIColor.dk_descriptionTextColor, NSFontAttributeName: UIFontMake(11), NSParagraphStyleAttributeName: [NSMutableParagraphStyle qmui_paragraphStyleWithLineHeight:12 lineBreakMode:NSLineBreakByTruncatingTail textAlignment:NSTextAlignmentCenter]}];
    UIImage *image = (UIImage *)self.dataSource[keyName];
    image = [image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    
    DKBaseGridButton *button = [[DKBaseGridButton alloc] init];
    button.tintColor = UIColor.dk_gridItemTintColor;
...
- (void)handleGirdButtonEvent:(DKBaseGridButton *)button {
...
@implementation DKBaseGridViewController (UISubclassingHooks)
...
@implementation DKBaseGridButton
