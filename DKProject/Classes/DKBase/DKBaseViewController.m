...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = HexColor(COLOR_WHITE);
    [self __addSubViews];
    [self __makeConstraints];
    [self __addViewModel];
    self.showBigTitle = NO;
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(noNetwork) name:Notification_NoNetwork_NameKey object:nil];
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(networkError) name:Notification_NetworkError_NameKey object:nil];
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(networkSuccess) name:Notification_NetworkSuccess_NameKey object:nil];
}
...
- (void)viewWillLayoutSubviews {
    ...
    if (@available (iOS 13.0, *)) {
        ...
        if (self.showBigTitle) {
            for (UIView *view in self.dk_Navi.navigationBar.subviews) {
                if ([view isKindOfClass:NSClassFromString(@"_UINavigationBarLargeTitleView")]) {
                    view.backgroundColor = HexColor(COLOR_WHITE);
...
//    }
//    self.tableView.emptyView.emptyImage = dkIMAGE(@"default_img_noNetwork");
...
//        self.emptyOriginalPrompt = self.tableView.emptyView.emptyPrompt;
//    }
//    self.tableView.emptyView.emptyImage = dkIMAGE(@"default_img_networkError");
